...
exports.TextSelectMenuBuilder = class TextSelectMenuBuilder {
  constructor () {
    /**
    ...
    /**
     *
     * @param {string | object} e - The element string or object
    ...
    this.formatElement = (e, i) => {
      if (typeof e === 'object') {
        return i
          ? `${i}° ${e.description}`
          : e.description || 'UNKNOWN_OBJECT_ELEMENT_DESCRIPTION'
      }
  ...
  addElement (element) {
    if (typeof element === 'string' || typeof element === 'object') {
      if (
        ...
        !this.elements.every((e) => typeof e === typeof element)
      ) {
        console.warn(
          'Trying to add different element type to an array already made out of ' +
          typeof element
        )
        return false
      }
      ...
      if (
        ...
        element.length > this.charsPerElement
      ) {
        console.warn(
  ...
  get pages () {
    ...
    for (let i = 0; i < this.elements.length; i++) {
      ...
      const elementString = this.formatElement(currentElement, i + 1) // TODO: This can fail with no error.
      let hasPushed = false
      ...
      individualElements[individualPageIndex].push(
        typeof currentElement === 'object' ? currentElement : elementString
      )
      pageContent += `\n${elementString}`
  ...
  get pageComponents () {
    ...
    const leastPageNum = (this.page - 1) < 0 ? 0 : this.page - 1
    const maxPageNum =
      this.page + 1 > pageCount - 1 ? pageCount - 1 : this.page + 1
    ...
    const backButton = new ButtonBuilder()
      ...
      .setCustomId(
        'back' +
          ...
          leastPageNum +
          (this.pageSwitchArgument
            ? `${this.separator + this.pageSwitchArgument}`
            : '')
      )
      .setStyle(ButtonStyle.Primary)
    ...
    if (leastPageNum !== maxPageNum) {
      const buttonsComponents = new ActionRowBuilder().addComponents(
        backButton,
        nextButton
      )
    ...
    if (this.supportLookUp) {
      ...
      let minIndex =
        this.elementsPerPage * (this.page + 1) - this.elementsPerPage
      if (this.page >= 1) {
