  ...
  async lookUp({ interaction, commandArguments }) {
    ...
    const buttons = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        ...
        .setStyle(ButtonStyle.Link),
      new ButtonBuilder().setURL(archiveEngineLink[engine]).setLabel('Engine Page').setStyle(ButtonStyle.Link),
      new ButtonBuilder().setLabel('Another Theme').setStyle(ButtonStyle.Primary).setCustomId(`themechange+${engine}`)
    );

    if (themeData.Date) themeEmbed.addFields({ name: 'Creation Date', value: themeData.Date, inline: true });
    if (themeData.Author) themeData.addFields({ name: 'Theme Author', value: themeData.Author, inline: true });
    if (themeData.Version) themeData.addFields({ name: 'Theme Version', value: themeData.Version, inline: true });
