...
exports.run = async (message, language) => {
    const enabledButton = new MessageActionRow().addComponents(
        buttonFile.quickButton(
            `retry${message.id}`,
            language.readLine('ping', 'Retry'),
            'PRIMARY'
        )
    )
    ...
    const test = async (msg, interaction) => {
        ...
        await msg.edit({
            content: language.readLine('ping', 'Ping'),
            components: [disabledButton],
        })
        await interaction.update({
            content: language.readLine('ping', 'Pong', {
                ms: Date.now() - before,
            }),
            components: [],
        })
    ...
    const filter = (i) =>
        i.customId === `retry${message.id}` && i.user.id === message.author.id
