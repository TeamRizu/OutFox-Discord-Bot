...
exports.run = async (message, language, { Sheet }) => {
    ...
    const userDefined = rows.find(
        (element) => element.user === message.author.id
    )

    const setLanguage = buttons.quickButton(`setlang${message.id}`, language.readLine('userlang', 'SetMyLanguage'), 'PRIMARY')
    const deleteMyLanguage = buttons.quickButton(`deletelang${message.id}`, language.readLine('userlang', 'DeleteMyLanguage'), 'DANGER')
    const nevermind = buttons.quickButton(`nevermind${message.id}`, language.readLine('userlang', 'Nevermind'), 'SECONDARY')
                ...
                msg.edit({
                    embeds: [
                        new MessageEmbed().setTitle(
                            language.readLine('userlang', 'SetLanguageFromList')
                        ),
                    ],
