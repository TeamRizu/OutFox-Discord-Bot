...
const main = async () => {
  ...
  for (let i = 0; i < 8; i++) {
    if (cutLines.includes(i)) {
      background.scan(
        ...
        measureWidth,
        3,
        makeIteratorThatFillsWithColor(0xffffffff)
      );
      background.scan(
        44,
        31 + 64 * i,
  ...
  const timings = {
    ...
    192: [
      4, 192, 192, 64, 48, 192, 32, 192, 24, 64, 192, 192, 16, 192, 192, 64, 12,
      192, 32, 192, 48, 64, 192, 192, 8, 192, 192, 64, 48, 192, 32, 192, 12, 64,
      192, 192, 16, 192, 192, 64, 24, 192, 32, 192, 48, 64, 192, 192,
    ],
  };

  const calculateNoteY = (
    mode,
    style,
    measure,
    timing,
    ...
    switch (mode) {
      case "bm": {
        return (
          [476, 220][measure] -
          (timing === 4
            ? noteSpacing[timing + endChar] *
              (line / timings[curMeasure.length].length)
    ...
    for (let line = 0; line < curMeasure.length; line++) {
      ...
      for (let char = 0; char < curLine.length; char++) {
        ...
        switch (noteType) {
          ...
          case "2": // HoldTop
            {
              const holdTop = await NoteSkin.collectAsset(
                "holdTop",
                timing,
                endChar,
                noteType,
          ...
          case "4": // Roll
            {
              const rollTop = await NoteSkin.collectAsset(
                "rollTop",
                timing,
                endChar,
                noteType,
              ...
              const measureNote = await NoteSkin.collectMeasure(
                "tapNote",
                timing,
                endChar,
                noteType,
          ...
          case "3": // Roll/Hold Bottom
            {
              ...
              const [bodyName, bodyX, bodyY, noteTiming] =
                lastNoteByLane[curLane];
              const body = await NoteSkin.collectAsset(
                `${bodyName}Body`,
                timing,
          ...
          case "M": // Mine
            {
              const mine = await NoteSkin.collectAsset(
                `mine`,
                timing,
                endChar,
                noteType,
          ...
          case "D": // MineField End/Lift Hold End/ Lift Roll End
            {
              if (!(curLine[char + 1] === "L" || curLine[char + 1] === "M"))
                continue;
              ...
              if (curLine[char + 1] === "M") {
                const mineTop = await NoteSkin.collectAsset(
                  `mineTop`,
                  timing,
                  endChar,
                  noteType + "M",
                ...
                const measureMineTop = await NoteSkin.collectMeasure(
                  "mineTop",
                  timing,
                  endChar,
                  noteType,
                ...
                lastNoteByLane[curLane] = [
                  "mine",
                  noteX,
                  noteY + measureMineTop.height,
                ]; // Minefield Hook
              } else {
                // Now it can only be lift roll/hold
                const [type, bodyX, bodyY, timingLane] =
                  lastNoteByLane[curLane];
